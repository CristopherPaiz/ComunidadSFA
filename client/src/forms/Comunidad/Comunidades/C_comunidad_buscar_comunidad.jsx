...
const C_comunidad_buscar_comunidad = () => {
  ...
  const handleBuscarPorretiro = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/persona/getbycomunidad`, {
        ...
        body: JSON.stringify({
          idcomunidad: valueretiro.currentKey,
        }),
        credentials: "include",
      });
      if (!response.ok) {
      ...
      toast.success("Persona por comunidad filtrado con éxito");
      setLoading(false);
    } catch (error) {
  ...
  return (
    <div className="flex w-full flex-col h-screen ">
      ...
      <div className="grid mx-auto gap-6  w-11/12 sm:w-3/5 sm:grid-cols-2">
        <p className="font-bold text-[17px] sm:hidden -mb-2 ">Seleccione la comunidad:</p>
        <Select
        ...
        <>
          <div className="flex w-full flex-row flex-wrap gap-4 pb-5">
            {resultadosPersonas?.map((persona, idx) => (
              <div
                key={idx}
                className="m-auto min-w-[300px] w-[320px] max-w-[500px] sm:min-w-[500px] cursor-pointer dark:border-white border-1 rounded-xl"
                onClick={() => {
...
export default C_comunidad_buscar_comunidad;

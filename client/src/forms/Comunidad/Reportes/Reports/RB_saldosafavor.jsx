...
const styles = StyleSheet.create({
  ...
  en3: { width: "400px", height: "auto", maxHeight: "60px" },
});
      ...
      <Page size="LETTER" orientation="landscape" style={styles.page}>
        <View style={styles.section}>
          <Text style={{ fontSize: 20, marginBottom: 10, textAlign: "center" }}>
            Reporte saldos a favor de: {value?.nombre}
          </Text>
          <View style={styles.table}>
            {/* Encabezado de la tabla */}
            <View style={[styles.tableRow, styles.tableHeader]}>
              <View style={[styles.tableCol, styles.en0]}>
                <Text>Proveniencia</Text>
              </View>
            ...
            {data.map((item, index) => (
              <View key={index} style={styles.tableRow}>
                ...
                <View style={[styles.tableCol, styles.en1]}>
                  <Text>{item?.fecha ? formatfecha(item?.fecha) : ""}</Text>
                </View>
                <View style={[styles.tableCol, styles.en2]}>
...
const RB_saldosafavor = () => {
  ...
  const handleBuscarPorComunidad = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/IngresoSaldo/${seleccionado._id}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!response.ok) {
        throw new Error("Error al filtrar los ingresos de saldo del beneficiario", {});
      }
      const data = await response.json();
      setResultados(data.ingresos);
      setLoading(false);
    } catch (error) {
      toast.error("Error al filtrar los ingresos de saldo del benefiaciario");
      setLoading(false);
      console.log(error);
    }
  };
          ...
          {isMobile ? (
            <div className="p-8">
              ...
              <PDFDownloadLink
                ...
                fileName="reporteBenefiarioSaldos.pdf"
              >
...
export default RB_saldosafavor;
